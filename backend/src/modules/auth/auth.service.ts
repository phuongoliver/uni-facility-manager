...
export class AuthService {
    ...
    async validateOAuth2User(profile: any): Promise<User> {
        ...
        const fullName = profile.name || profile.full_name || `${profile.first_name} ${profile.last_name}`;
    ...
    async exchangeSsoToken(code: string) {
        try {
            const tokenUrl = process.env.OAUTH2_TOKEN_URL || 'https://devhcmutsso.namanhishere.com/oauth/token';
