...
export class PaymentsService {
    ...
    private pendingTransactions: Map<string, { bookingId: number; expires: Date; amount: number }> = new Map();
     ...
     */
    async initiatePayment(bookingId: number, userId: number): Promise<BKPayResponse> {
        ...
        if (booking.status !== BookingStatus.WAITING_PAYMENT) {
            throw new BadRequestException(`Booking must be WAITING_PAYMENT to initiate payment. Current: ${booking.status}`);
        }
     ...
     */
    async handleSuccessCallback(transactionRef: string, signature?: string): Promise<{ success: boolean; bookingId: number; message: string }> {
        const transaction = this.pendingTransactions.get(transactionRef);

        if (!transaction) {
            throw new BadRequestException(`Transaction ${transactionRef} not found or expired`);
        }

        if (new Date() > transaction.expires) {
            this.pendingTransactions.delete(transactionRef);
            throw new BadRequestException(`Transaction ${transactionRef} has expired`);
        }
        ...
        if (booking.groupId) {
            // Confirm Group
            await this.bookingsRepository.manager.update(BookingGroup, { groupId: booking.groupId }, { status: BookingStatus.CONFIRMED });
            // Confirm All
     ...
     */
    async handleCancelCallback(transactionRef: string): Promise<{ success: boolean; bookingId: number; message: string }> {
        const transaction = this.pendingTransactions.get(transactionRef);
     ...
     */
    async checkPaymentStatus(transactionRef: string): Promise<{ status: 'pending' | 'completed' | 'expired' | 'not_found'; bookingId?: number }> {
        const transaction = this.pendingTransactions.get(transactionRef);
     ...
     */
    async simulatePaymentComplete(transactionRef: string): Promise<{ success: boolean; message: string }> {
