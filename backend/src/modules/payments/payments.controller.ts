import { Controller, Post, Get, Param, Query, Headers as HttpHeaders, Body, Res } from '@nestjs/common';
...
export class PaymentsController {
    ...
    async successCallback(
        ...
        try {
            const result = await this.paymentsService.handleSuccessCallback(transactionRef, signature);
            // Redirect to frontend success page
            const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';
            return res.redirect(`${frontendUrl}/my-bookings?payment=success&booking=${result.bookingId}`);
        } catch (error) {
            const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';
            return res.redirect(`${frontendUrl}/my-bookings?payment=error&message=${encodeURIComponent(error.message)}`);
        }
    }
    ...
    async cancelCallback(
        ...
        try {
            const result = await this.paymentsService.handleCancelCallback(transactionRef);
            const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';
            return res.redirect(`${frontendUrl}/my-bookings?payment=cancelled&booking=${result.bookingId}`);
        } catch (error) {
            const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';
