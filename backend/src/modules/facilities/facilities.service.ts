...
export class FacilitiesService {
    ...
    async create(managerId: number, createFacilityDto: CreateFacilityDto): Promise<Facility> {
        ...
        if (existing) {
            throw new ConflictException(`Facility with name "${createFacilityDto.name}" already exists.`);
        }
    ...
    async update(id: number, managerId: number, updateDto: UpdateFacilityDto): Promise<Facility> {
        const facility = await this.facilitiesRepository.findOne({ where: { facilityId: id } });
        if (!facility) {
