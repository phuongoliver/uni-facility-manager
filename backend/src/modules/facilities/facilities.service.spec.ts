...
describe("FacilitiesService", () => {
  ...
  describe("create", () => {
    it("should create a facility successfully", async () => {
      ...
      expect(result).toEqual(
        expect.objectContaining({ name: "Test Hall", status: "AVAILABLE" }),
      );
    ...
    it("should throw ConflictException if duplicate name", async () => {
      ...
      jest
        ...
        .mockResolvedValue({ facilityId: 1, name: "Existing Hall" } as any);

      await expect(service.create(managerId, createDto)).rejects.toThrow(
        ConflictException,
      );
    });
  });
  ...
  describe("update", () => {
    it("should update facility if manager owns it", async () => {
      ...
      jest
        .spyOn(repository, "findOne")
        .mockResolvedValue(existingFacility as any);
      jest
        .spyOn(repository, "save")
        .mockResolvedValue({ ...existingFacility, ...updateDto } as any);
    ...
    it("should throw ForbiddenException if manager does not own it", async () => {
      ...
      await expect(
        service.update(facilityId, managerId, updateDto),
