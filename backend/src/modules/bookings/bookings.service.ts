...
export class BookingsService {
    ...
    private async _createBookingTransaction(
        ...
        if (start_slot > end_slot) {
            throw new BadRequestException(
                "Start slot must be before or equal to End slot",
            );
        }
            ...
            for (const item of equipment_items) {
                ...
                if (!eq)
                    throw new NotFoundException(
                        `Equipment ID ${item.equipment_id} not found`,
                    );
                if (eq.availableQuantity < item.quantity) {
                    throw new BadRequestException(
                        `Thiết bị '${eq.name}' không đủ số lượng. Kho còn: ${eq.availableQuantity}, Yêu cầu: ${item.quantity}`,
                    );
                }
        ...
        for (const date of datesToBook) {
            ...
            if (overlap) {
                ...
                throw new ConflictException(
                    `Phòng đã có người đặt vào ngày ${dateString} trong khung giờ này.`,
                );
            }
    ...
    async create(userId: number, createBookingDto: CreateBookingDto) {
        ...
        try {
            const resultBookings = await this._createBookingTransaction(
                queryRunner.manager,
                userId,
                createBookingDto,
            );
            ...
            if (finalBooking) {
                await this.notificationsService.send(
                    userId,
                    "Booking Received",
                    `Booking for ${finalBooking.facility?.name} is pending approval.`,
                );
                if (finalBooking.facility?.managerId) {
                    await this.notificationsService.sendAll(
                        finalBooking.facility.managerId,
                        "New Booking Request",
                        `New booking request for ${finalBooking.facility.name} needs approval.`,
                    );
                }
            }
            return finalBooking;
            ...
            if (err.code === "23P01") {
                throw new ConflictException(
                    "Xung đột lịch đặt phòng (Race condition).",
                );
            }
            throw err;
        } finally {
    ...
    async reschedule(
        ...
        try {
            ...
            if (!oldBooking)
                throw new NotFoundException(`Booking #${oldBookingId} not found`);

            // Only CONFIRMED bookings can be rescheduled
            if (oldBooking.status !== BookingStatus.CONFIRMED) {
                throw new BadRequestException(
                    `Only CONFIRMED bookings can be rescheduled. Current status: ${oldBooking.status}. Please cancel and create a new booking instead.`,
                );
            }
            ...
            const existingReschedule = await queryRunner.manager.findOne(Booking, {
                where: {
                    parentBookingId: oldBookingId,
                    status: BookingStatus.PENDING_RESCHEDULE,
                },
            });
            if (existingReschedule) {
                throw new BadRequestException(
                    `A reschedule request for this booking is already pending (Booking #${existingReschedule.bookingId}).`,
                );
            }
            ...
            if (final) {
                await this.notificationsService.send(
                    ...
                    `Reschedule request for ${final.facility?.name} submitted.`,
                );
                if (final.facility?.managerId) {
                    await this.notificationsService.sendAll(
                        final.facility.managerId,
                        "New Reschedule Request",
                        `A reschedule request for ${final.facility.name} needs review.`,
                    );
                }
            }
            return final;
            ...
            await queryRunner.rollbackTransaction();
            if (err.code === "23P01")
                throw new ConflictException("Xung đột lịch đặt phòng.");
            throw err;
        } finally {
            await queryRunner.release();
        }
    ...
    async approveReschedule(rescheduleBookingId: number, _managerId: number) {
        ...
        if (!rescheduleBooking)
            throw new NotFoundException(`Booking #${rescheduleBookingId} not found`);
        if (rescheduleBooking.status !== BookingStatus.PENDING_RESCHEDULE) {
            throw new BadRequestException(
                `Booking must be PENDING_RESCHEDULE to approve. Current: ${rescheduleBooking.status}`,
            );
        }
        if (!rescheduleBooking.parentBookingId) {
            throw new BadRequestException(
                `This booking is not a reschedule request (no parent booking).`,
            );
        }
        ...
        if (conflict && conflict.bookingId !== rescheduleBooking.parentBookingId) {
            throw new ConflictException(
                `Facility is already booked during this time (Booking #${conflict.bookingId}).`,
            );
        }
        ...
        try {
            ...
            const facilityPrice = parseFloat(
                rescheduleBooking.facility.price?.toString() || "0",
            );
            if (facilityPrice === 0) {
            ...
            await this.notificationsService.sendAll(
                result.userId,
                "Reschedule Approved",
                `Your reschedule request for ${rescheduleBooking.facility?.name} has been approved.`,
            );
    ...
    async rejectReschedule(
        ...
        if (rescheduleBooking.status !== BookingStatus.PENDING_RESCHEDULE) {
            throw new BadRequestException(
                `Booking must be PENDING_RESCHEDULE to reject. Current: ${rescheduleBooking.status}`,
            );
        }

        rescheduleBooking.status = BookingStatus.REJECTED;
        rescheduleBooking.cancellationReason =
            reason || "Reschedule request rejected by manager";
        ...
        await this.notificationsService.sendAll(
            saved.userId,
            "Reschedule Rejected",
            `Reschedule request rejected. Reason: ${saved.cancellationReason}`,
        );
        return saved;
    }
    ...
    async cancel(id: number, userId: number, reason: string) {
        ...
        if (!isManager) {
            ...
            if (timeDiff < minDiff) {
                throw new BadRequestException(
                    `Cancellation deadline passed. Must cancel ${minHours} hours in advance.`,
                );
            }
        }
        ...
        if (!isManager && cancelledBooking.facility?.managerId) {
            await this.notificationsService.send(
                cancelledBooking.facility.managerId,
                "Booking Cancelled",
                `Booking #${cancelledBooking.bookingId} for ${cancelledBooking.facility.name} was cancelled by user.`,
            );
        }
    ...
    async approve(id: number, managerId: number) {
        ...
        if (booking.status !== BookingStatus.PENDING) {
            throw new BadRequestException(
                `Booking must be PENDING to approve. Current: ${booking.status}`,
            );
        }
        ...
        if (booking.groupId) {
            ...
            for (const b of groupBookings) {
                ...
                if (conflict) {
                    throw new ConflictException(
                        `Cannot approve group: Slot ${b.checkInTime.toISOString()} is already booked (Booking #${conflict.bookingId}).`,
                    );
                }
            }
            ...
            const facilityPrice = parseFloat(
                booking.facility.price?.toString() || "0",
            );
            const newStatus =
                facilityPrice === 0
                    ? BookingStatus.CONFIRMED
    ...
    async confirmPayment(id: number, userId: number) {
        ...
        if (!isOwner && !isManager) {
            throw new BadRequestException(
                `Not authorized to confirm payment for this booking`,
            );
        }
        ...
        await this.notificationsService.send(
            booking.userId,
            "Payment Successful",
            `Your booking (and any recurring ones) for ${booking.facility.name} is now CONFIRMED.`,
        );

        // Notify Manager if they didn't trigger it
        if (booking.facility?.managerId && booking.facility.managerId !== userId) {
            await this.notificationsService.send(
                booking.facility.managerId,
                "Payment Confirmed",
                `Payment for booking #${booking.bookingId} confirmed.`,
            );
        }
